    ...
    contactForm.addEventListener(
      ...
      (event) => {
        ...
        fetch(contactForm.action, {
          ...
          if (response.ok) {
            showToast(toastAnchor, "Success! Your message has been sent.", "success");
            contactForm.reset();
            contactForm.classList.remove("was-validated");
          } else {
            response.json().then(data => {
              if (Object.prototype.hasOwnProperty.call(data, 'errors')) {
                showToast(toastAnchor, data["errors"].map(error => error["message"]).join(", "), "danger");
              } else {
                showToast(toastAnchor, "Oops! There was a problem submitting your form", "danger");
              }
            });
          }
        }).catch(error => {
